		...
		function exportAsTwoPages(){
			...
			window.html2canvas(clone, {
				...
				try{
					...
					// Evita topo em branco no PDF quando o ponto de corte fica antes do limite
					// natural da primeira fatia (valores negativos deslocam o conteúdo para baixo).
					var maxStartY = Math.max(0, canvas.height - sliceHeightPx);
					if (page1StartY < 0) page1StartY = 0;
					if (page1StartY > maxStartY) page1StartY = maxStartY;
					if (page2StartY < 0) page2StartY = 0;
					if (page2StartY > maxStartY) page2StartY = maxStartY;
					if (page1StartY === 0) {
						var minSecondStart = Math.max(0, sliceHeightPx - overlapPx);
						if (maxStartY >= minSecondStart && page2StartY < minSecondStart) {
							page2StartY = minSecondStart;
						}
					}
					...
					doc.addImage(imgData1, 'JPEG', xMm, marginTopMm, drawWidthMm, usableHeightMm);
					doc.addPage();
					var sliceCanvas2 = makeSlice(page2StartY);
					var imgData2 = sliceCanvas2.toDataURL('image/jpeg', jpgQuality);
					doc.addImage(imgData2, 'JPEG', xMm, marginTopMm, drawWidthMm, usableHeightMm);
