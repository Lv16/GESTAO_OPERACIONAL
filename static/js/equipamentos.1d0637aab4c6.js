...
(function(){
                            ...
                            if (data && data.success) {
                                ...
                                try {
                                    ...
                                    if (tb && data.equipamento && data.formulario) {
                                        ...
                                        tr.appendChild(cell((form.querySelector('[name="situacao"]')||{value:''}).value || '', 'Situação'));
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Inline situacao update handler (selects inside table rows)
        document.querySelectorAll('.situacao-select').forEach(sel => {
            sel.addEventListener('change', async (e) => {
                const select = e.currentTarget;
                const val = select.value || '';
                const equipId = select.getAttribute('data-equip-id');
                if (!equipId) return;
                // CSRF cookie helper
                ...
                try {
                    ...
                    if (!resp.ok) { showToast('error', 'Erro ao atualizar situação'); return; }
                    const data = await resp.json();
                    if (data && data.success) {
                        // update row attribute
                        const tr = select.closest && select.closest('tr'); if (tr) tr.setAttribute('data-situacao', val);
                        // optionally, we could fetch situacao_history from response and display it later
                        if (data.situacao_history && Array.isArray(data.situacao_history)) {
                            // for now just log to console; UI enhancements can display it
                            console.debug('situacao_history', data.situacao_history);
                        }
                        showToast('success', 'Situação atualizada.');
                    } else {
                        showToast('error', 'Erro ao atualizar situação: ' + (data && data.error ? data.error : 'resposta inválida'));
                    }
