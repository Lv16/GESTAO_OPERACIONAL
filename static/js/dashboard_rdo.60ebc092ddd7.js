...
function resetFilters() {
    ...
    let startDate = null;
    try{
        const rawStart = String((window && window.DASHBOARD_DEFAULT_START) ? window.DASHBOARD_DEFAULT_START : '').trim();
        const m = rawStart.match(/^(\d{4})-(\d{2})-(\d{2})$/);
        if(m){
            const y = Number(m[1]);
        ...
        function updateInsightsAndHelp(selectedTank){
            ...
            try{
                ...
                const scopeTxt = (osStatusScope === 'em_andamento')
                    ...
                    : '';
                help.textContent = `Carrossel automático por tanque (${__tempo_bomba_view_state.intervalSec}s) · Barras = período do tanque · Linha sólida = curva S do tanque · Tracejado = previsão por tanque${scopeTxt}${hiddenTxt}`;
            }catch(e){ /* ignore */ }
        }
