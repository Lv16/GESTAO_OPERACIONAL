...
function renderMetodoEficaciaLegend(items, colors, totalIndice, mediaConclusao){
    ...
    if(!Array.isArray(items) || !items.length){
        ...
        if(helpEl){
            helpEl.textContent = 'Sem dados suficientes para montar o gráfico de eficácia.';
        }
        return;
    }
    ...
    if(helpEl){
        helpEl.textContent = `Centro do gráfico: média ponderada da taxa de conclusão (${formatPercentPt(mediaSafe, 1)}%).`;
    }
}
