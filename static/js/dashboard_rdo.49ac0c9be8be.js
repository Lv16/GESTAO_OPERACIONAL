...
async function loadChartTempoBomba(filters) {
    try {
        ...
        Object.entries(tankProgressMetaRaw).forEach(([label, info]) => {
            ...
            const remaining = isFinite(remainingRaw) ? remainingRaw : (isFinite(done) ? (100 - done) : null);
            tankProgressByKey[key] = {
                donePercent: isFinite(done) ? done : null,
                remainingPercent: isFinite(remaining) ? Math.max(0, remaining) : null,
        ...
        tanks = tanks.map(t => {
            ...
            return {
                ...
                progressDone: isFinite(progress.donePercent) ? progress.donePercent : null,
                progressRemaining: isFinite(progress.remainingPercent) ? progress.remainingPercent : null,
        ...
        function updateInsightsAndHelp(selectedTank){
            ...
            if(selectedTank && isFinite(selectedTank.progressDone) && isFinite(selectedTank.progressRemaining)){
        ...
        function renderControls(ui){
            ...
            tanks.forEach((tank, i) => {
                ...
                const missingText = isFinite(tank.progressRemaining) ? ` · falta ${formatPercent(tank.progressRemaining)}` : '';
            ...
            const missingCurrent = (current && isFinite(current.progressRemaining)) ? ` · falta ${formatPercent(current.progressRemaining)}` : '';
    ...
    async function loadChartPobComparativo(filters) {
        try {
            ...
            const prepared = {
                ...
                datasets: [
                    Object.assign({}, ds[0] || {}, { label: ds[0]?.label || 'POB Alocado (média)', backgroundColor: '#2563eb', borderColor: '#2563eb', borderRadius: 0, maxBarThickness: 48, barPercentage: 0.7, categoryPercentage: 0.7, order: 2 }),
                    Object.assign({}, ds[1] || {}, { label: ds[1]?.label || 'POB em Espaço Confinado (média)', backgroundColor: '#f59e0b', borderColor: '#f59e0b', borderRadius: 6, maxBarThickness: 48, barPercentage: 0.7, categoryPercentage: 0.7, order: 1 })
                ]
            };
