    ...
    try {
        ...
        if(tanks.length){
            ...
            if(!hasCurrent){
                ...
                if(__tempo_bomba_view_state.mode === 'fixed'){
        ...
        if(__tempo_bomba_view_state.mode !== 'auto' && __tempo_bomba_view_state.mode !== 'fixed'){
            __tempo_bomba_view_state.mode = 'auto';
        }
        ...
        function buildSingleTankChart(tank){
            ...
            const maxCum = Math.max(0, ...tankCumulative, ...totalCumulative);
                    ...
                    tooltip: {
                        callbacks: {
                            ...
                            footer: function(items){
                                ...
                                const totalPoint = Number(barsTotal[idx]) || 0;
                                return [
                                    `Tanque no período: ${formatHoursToHHMM(tankPoint)}`,
                                    `Total no período: ${formatHoursToHHMM(totalPoint)}`
                                ];
                            }
                        }
                    },
        ...
        function updateInsightsAndHelp(selectedTank){
            const modeText = (() => {
                if(__tempo_bomba_view_state.mode === 'fixed'){
                    return `${modeLabel} · Tanque fixo`;
                }
                return `${modeLabel} · Carrossel ${__tempo_bomba_view_state.intervalSec}s`;
            })();
            ...
            try{
                ...
                if(__tempo_bomba_view_state.mode === 'fixed'){
                    help.textContent = `Barras = horas do tanque no período · Linha sólida = curva S do tanque · Tracejado = curva S total (${modeLabel.toLowerCase()})${hiddenTxt}`;
                } else {
                    help.textContent = `Carrossel automático por tanque (${__tempo_bomba_view_state.intervalSec}s) · pausa ao passar o mouse · clique em um tanque para fixar${hiddenTxt}`;
                }
            }catch(e){ /* ignore */ }
        }
        ...
        function renderControls(ui){
            ...
            tanks.forEach(tank => {
                const active = __tempo_bomba_view_state.currentTankLabel === tank.label;
                const pill = makeTankPill(tank, active);
                pill.onclick = function(){
                    __tempo_bomba_view_state.mode = 'fixed';
                    __tempo_bomba_view_state.paused = false;
                    __tempo_bomba_view_state.currentTankLabel = tank.label;
                    renderFrame();
                    restartCarousel();
                };
        ...
        function restartCarousel(){
            ...
            if(__tempo_bomba_view_state.mode !== 'auto' || tanks.length <= 1) return;
            const everyMs = Math.max(15, Number(__tempo_bomba_view_state.intervalSec) || 60) * 1000;
