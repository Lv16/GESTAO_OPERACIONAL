...
async function loadChartMetodoEficacia(filters) {
    try {
        ...
        const values = data.efficacy_index || [];
        const finalizadas = data.finalizadas_counts || [];
        const andamento = data.andamento_counts || [];
        const avgFinal = data.avg_days_finalizadas || [];
        const avgAnd = data.avg_days_andamento || [];

        const maxEfficacy = Math.max(0, ...values.map(v => Number(v || 0)));
        const valuesRelative = values.map((v) => {
            const num = Number(v || 0);
            if (maxEfficacy <= 0) return 0;
            return Number(((num / maxEfficacy) * 100).toFixed(2));
        });

        const taxasConclusao = labels.map((_, idx) => {
            const fin = Number(finalizadas[idx] || 0);
            const andm = Number(andamento[idx] || 0);
            const total = fin + andm;
            if (!total) return 0;
            return Number(((fin / total) * 100).toFixed(2));
        });

        const indexColor = 'rgba(249, 115, 22, 0.95)';
        const indexBorderColor = 'rgba(234, 88, 12, 1)';
        ...
        const options = {
                ...
                tooltip: {
                    callbacks: {
                        label: function(ctx) {
                            ...
                            const parsedVal = ctx.parsed && (ctx.parsed.x !== undefined ? ctx.parsed.x : ctx.parsed.y);
                            const val = Number(parsedVal || 0);
                            const fin = finalizadas[idx] || 0;
                            const andm = andamento[idx] || 0;
                            const avgF = (avgFinal[idx] === null || avgFinal[idx] === undefined) ? '-' : avgFinal[idx];
                            const avgA = (avgAnd[idx] === null || avgAnd[idx] === undefined) ? '-' : avgAnd[idx];
                            if (ctx.dataset && ctx.dataset.label && ctx.dataset.label.includes('Taxa')) {
                                return `Taxa de conclusão: ${val.toFixed(2)}%`;
                            }
                            const rawScore = Number(values[idx] || 0).toFixed(2);
                            return `Score relativo: ${val.toFixed(2)}% | Índice real: ${rawScore} | Finalizadas: ${fin} | Em andamento: ${andm} | Média finalizadas: ${avgF} dias | Média em andamento: ${avgA} dias`;
                        }
                    }
                }
            },
            scales: {
                x: {
                    beginAtZero: true,
                    max: 100,
                    title: { display: true, text: 'Score relativo (%)' },
        ...
        updateChart('chartMetodoEficacia', 'bar', prepared, options);
...
async function loadChartLiquidoSupervisor(filters) {
    try {
        ...
        const prepared = { labels: sortedLabels, datasets: [{ label: ds.label || 'M³ líquido removido', data: sortedValues, backgroundColor: '#2563eb' }] };
