...
async function loadChartLiquidoSupervisor(filters) {
    try {
        ...
        // Dividir valores por 100 para exibir em centenas de M³
        const sortedValues = pairs.map(p => Math.round((p.value / 100) * 100) / 100);
...
async function loadChartSolidoSupervisor(filters) {
    try {
        ...
        const prepared = { labels: sortedLabels, datasets: [{ label: ds.label || 'M³ sólido removido (x100)', data: sortedValues, backgroundColor: '#6fbf4f' }] };
