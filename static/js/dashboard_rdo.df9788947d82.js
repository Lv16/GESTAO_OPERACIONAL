    ...
    try {
        ...
        const results = await Promise.all([
            ...
            loadChartPobComparativo(filters),
                ...
                loadChartEntradaSaidaSemanalCoordenador(filters),
...
async function loadChartLiquidoSupervisor(filters) {
    try {
        ...
        const sortedValues = filtered.map(p => Math.round(p.value / 100));
...
async function loadChartEntradaSaidaSemanalCoordenador(filters){
    const metaEl = document.getElementById('chartEntradaSaidaCoordenadorMeta');
    try{
        const data = await fetchChartData('/api/rdo-dashboard/entrada_saida_semanal_coordenador/', filters);
        if(!data || !data.success){
            throw new Error((data && data.error) || 'Erro ao obter entrada/saida semanal por coordenador');
        }

        const labels = Array.isArray(data.labels) ? data.labels : [];
        const entradasSem = Array.isArray(data.entradas_semanais) ? data.entradas_semanais : [];
        const saidasSem = Array.isArray(data.saidas_semanais) ? data.saidas_semanais : [];
        const saldoSem = Array.isArray(data.saldo_semanal) ? data.saldo_semanal : [];
        const entradasTot = Array.isArray(data.entradas_total) ? data.entradas_total : [];
        const saidasTot = Array.isArray(data.saidas_total) ? data.saidas_total : [];
        ...
        const options = {
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(ctx){
                            const idx = Number(ctx.dataIndex || 0);
                            const dsLabel = String(ctx.dataset && ctx.dataset.label || '');
                            const value = Number(ctx.parsed && (ctx.parsed.y !== undefined ? ctx.parsed.y : ctx.parsed.x) || 0);
                            if(dsLabel === 'Entradas/semana'){
                                const total = Number(entradasTot[idx] || 0);
                                return `${dsLabel}: ${formatNumberPt(value, 2)} (total: ${Intl.NumberFormat('pt-BR').format(total)})`;
                            }
                            if(dsLabel === 'Saidas/semana'){
                                const total = Number(saidasTot[idx] || 0);
                                return `${dsLabel}: ${formatNumberPt(value, 2)} (total: ${Intl.NumberFormat('pt-BR').format(total)})`;
                            }
                            return `${dsLabel}: ${formatNumberPt(value, 2)}`;
                        }
                    }
                },
                barValuePlugin: { display: false }
            ...
            scales: {
                y: {
                    beginAtZero: true,
                    title: { display: true, text: 'OS por semana (média)' },
                    grid: { color: 'rgba(148, 163, 184, 0.16)' }
                },
                x: {
                    ticks: {
        ...
        options.__preserveDatasetColors = true;
        updateChart('chartEntradaSaidaCoordenador', 'bar', prepared, options);

        if(metaEl){
            metaEl.textContent = `Média semanal no período (${formatNumberPt(Number(data.weeks_equivalent || 0), 2)} semanas equivalentes).`;
        }
        return { key: 'entrada_saida_semanal_coordenador', data };
    }catch(error){
        console.error('Erro ao carregar Entrada x Saída Semanal por Coordenador:', error);
        if(metaEl){
            metaEl.textContent = 'Sem dados disponíveis para os filtros aplicados.';
        }
        updateChart('chartEntradaSaidaCoordenador', 'bar', { labels: [], datasets: [{ label: 'Fluxo semanal', data: [] }] }, {
            plugins: { legend: { display: false }, barValuePlugin: { display: false } },
