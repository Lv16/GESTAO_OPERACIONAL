    ...
    try {
        ...
        function renderControls(ui){
            ...
            tanks.forEach((tank, i) => {
                ...
                const missingText = Number.isFinite(tank.progressRemaining) ? ` · falta ${formatPercent(tank.progressRemaining)}` : '';
            ...
            const missingCurrent = (current && Number.isFinite(current.progressRemaining)) ? ` · falta ${formatPercent(current.progressRemaining)}` : '';
...
async function loadChartVolumeTanque(filters) {
    ...
    async function loadChartPobComparativo(filters) {
        try {
            ...
            // Filtrar dias sem nenhum POB (todos os datasets com valor 0 ou nulo)
            const filteredIndices = [];
            const filteredLabels = [];
            (prepared.labels || []).forEach((label, i) => {
                const hasPob = prepared.datasets.some(ds => {
                    const v = Number(ds.data && ds.data[i] || 0);
            ...
            if (filteredIndices.length > 0 && filteredIndices.length < (prepared.labels || []).length) {
